...
- Validation
  ...
  - Claim payloads now count against both limits. A payload is arbitrary JSON, and `measure()` stopped at claim nodes, so a deeply nested one raised `RangeError: Maximum call stack size exceeded` out of a function documented to return a validation result.
