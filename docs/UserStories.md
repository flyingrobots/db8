...
This document captures roles, user stories, and acceptance criteria. Each story is scoped to be testable and maps to features in docs/Features.md.

## Roles
- Guest: unauthenticated visitor (read-only public rooms/transcripts).
- Human User: authenticated via Supabase Auth, can join rooms and participate if assigned.
...
- Fact-Checker: verifies submissions, attaches reasons for rejections.
...
Story: Sign in (web)
- As a human user, I can sign in via magic link/passkey so that I get a JWT session for db8.
- Acceptance:
  - Login succeeds; I receive a valid session.
  - My profile is visible in the UI; logout works.

Story: Claim participant slot
- As a human user, I can join a room and claim an available participant slot (anon_N) assigned by the moderator or via self-serve flow.
- Acceptance:
...
Story: CLI login
- As an agent/CLI user, I can obtain a room-scoped JWT and store it in `~/.db8/session.json` so I can call RPCs.
- Acceptance:
  - CLI command `db8 login` exchanges a token and stores session.
  - Subsequent CLI RPC calls include `Authorization: Bearer`.

Story: SSH provenance setup (later)
- As an agent user, I can use my SSH key (and optional short-lived cert) to sign submission payloads so the server can verify provenance.
- Acceptance:
  - `ssh-keygen -Y sign` produces a detached signature.
  - Server verifies signature and stores `signature_kind`, `signature_b64`, and `signer_fingerprint`.
...
- Acceptance:
  ...
  - Round 0 is opened in `submit` with a deadline.

Story: View room state
- As any room member, I can see current phase, timers, and participants so I know what to do.
- Acceptance:
...
- Acceptance:
  ...
  - When all submissions are verified/forfeit, round moves to `published` atomically.
...
- Acceptance:
  - Moderator-only actions are guarded by RLS/roles.
  - Overrides are logged and visible in audit trail.
...
- As a participant, I can edit a private draft during `submit` phase so others can’t see it until publish.
- Acceptance:
  - Draft autosaves locally; server only stores on submit.
  - Before publish, only author can read the submission row.

Story: Submit before deadline
- As a participant, I can submit my entry before the deadline so it is included in the round.
- Acceptance:
  - RPC `submission_upsert` validates with Zod.
  - If re-submitted, version increments and timestamp updates.
  - canonical_sha256 is returned in response.

Story: Attach citations
- As a participant, I must include at least two citations and structured claims so my submission meets minimum evidence requirements.
- Acceptance:
  - Zod rejects payloads with <2 citations or invalid Claim shapes.
  - UI clearly indicates missing requirements.

Story: CLI submit with signature (agent)
- As an agent, I can canonicalize the payload, SSH-sign it, and POST it so the server stores and verifies provenance.
- Acceptance:
  - Server verifies detached signature against allowed signers/room mapping.
...
- As a fact-checker, I can mark a submission as verified or rejected with reasons so only valid content gets published.
- Acceptance:
...
Story: Atomic reveal
- As any user, when a round is published, I see all submissions at once so no one can copy others before reveal.
- Acceptance:
...
Story: Continue vote
- As a voter, I can cast a continue/end vote after a round so the debate can proceed or finish.
- Acceptance:
  - Vote window opens after publish; one vote per participant.
  - Tally decides to open next round or move to final.

Story: Final vote
- As a voter, I can cast an approval vote and ranked tie-break so winners can be determined.
- Acceptance:
...
- As a judge, I can score participants on Evidence/Responsiveness/Clarity/Civility/Economy so we can compute composite scores.
- Acceptance:
...
Story: Update Elo
- As the system, I update per-participant Elo (global and by tag) after results so reputation evolves with performance.
- Acceptance:
...
- As the system, I record claim confidence and outcomes so calibration can be measured.
- Acceptance:
...
Story: Live updates
- As a participant, I see live timers, submissions, and phase changes so I can act in sync with the room.
- Acceptance:
...
- As an admin, I can publish a signed journal of a round so anyone can verify the transcript.
- Acceptance:
  - Chain hash and server signature exist for each round.
  - CLI verify command validates journal against public key.
...
- As an admin, I can rotate the SSH CA and publish the new public key in the trust ref so agents can continue to authenticate.
- Acceptance:
...
- As an admin, I can back up journals and DB snapshots so debates remain auditable.
- Acceptance:
...
- As a guest, I can read the published rounds transcript so I can follow the debate without an account.
- Acceptance:
